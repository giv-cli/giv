...
provider_custom_get_version() {
    ...
    # out=$(cat "$GIV_PROJECT_VERSION_FILE" | sed -n -E 's/.*([vV][0-9]+\.[0-9]+\.[0-9]+).*/\1/p')
    # if [ -z "$out" ]; then
    #     out=$(cat "$GIV_PROJECT_VERSION_FILE" | sed -n -E 's/.*([0-9]+\.[0-9]+\.[0-9]+).*/\1/p')
    # fi
    # printf '%s' "$out"
    if [ -z "$GIV_PROJECT_VERSION_FILE" ]; then
        printf ""
        return 0
    fi
    if [ ! -f "$GIV_PROJECT_VERSION_FILE" ]; then
        printf ""
        return 0
    fi
    # If file is JSON, extract "version" field (case-insensitive)
    if grep -i -q 'version' "$GIV_PROJECT_VERSION_FILE" 2>/dev/null; then
        #print_debug "Extracting version from file: $GIV_PROJECT_VERSION_FILE"
        version=$(grep -i 'version' "$GIV_PROJECT_VERSION_FILE" | sed -En 's/.*[vV]ersion"[[:space:]]*:[[:space:]]*([^\"]+)".*/\1/p' | head -n 1)
        # If version is empty or does not match a version pattern, fallback
        if [ -z "$version" ]; then
            parse_version "$(cat "$GIV_PROJECT_VERSION_FILE")"
        else
            printf '%s' "$version"
        fi
    else
...
provider_custom_get_version_at_commit() {
    ...
    file_content=$(git show "${commit}:${GIV_PROJECT_VERSION_FILE}" 2>/dev/null)
